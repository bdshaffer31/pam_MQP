...
void loop() {
  ...
  if (myIMU.readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)
  {
    ...
    lastTime = millis();
    int lastStatus = rollStatus;
    checkPluck(lastStatus);
...
void updateVelocity(float deltat) {
  ...
  vx = vx + ((int)1000 * trimmedAcceleration(myIMU.ax, "x")) * deltat / 1000;
  vy = vy + ((int)1000 * trimmedAcceleration(myIMU.ay, "y")) * deltat / 1000;
  vz = vz + ((int)1000 * trimmedAcceleration(myIMU.az, "z")) * deltat / 1000;
}

float trimmedAcceleration(float accel, String axis){
  if (axis == "x") {
    return tolerancedAcceleration(accel, axis) - xAclAdjust;
  } else if (axis == "y") {
    return tolerancedAcceleration(accel, axis) - yAclAdjust;
  } else if (axis == "z") {
    return tolerancedAcceleration(accel, axis) - zAclAdjust;
  } else {
    return 999.999;
  }
}
...
void checkPluck(int lastStatus){
  rollStatus = updateStatus();
  if (rollStatus != lastStatus){
    Serial.println(rollStatus);
  }
}

int updateStatus(){
  if (abs(myIMU.roll) < stringAngle){
    return 0;
  } else if (abs(myIMU.roll) < 2*stringAngle){  
    return 1;  
  }else if (abs(myIMU.roll) < 3*stringAngle){
    return 2;
  }else if (abs(myIMU.roll) < 4*stringAngle){
    return 3;  
  }else if (abs(myIMU.roll) < 5*stringAngle){
     return 4; 
  }else if (abs(myIMU.roll) < 6*stringAngle){
    return 5; 
  }else{
     return 6;
  }
